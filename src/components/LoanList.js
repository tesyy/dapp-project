...
const LoanList = ({ loans, contract, address, onTransactionComplete }) => {
  ...
  useEffect(() => {
    console.log("LoanList component rendered with props:", { 
      loansCount: loans?.length, 
      contract: !!contract,
      address: address,
    });
    
    // Log each loan's repayment eligibility
    loans.forEach(loan => {
      ...
      const isNotRepaid = !loan.repaid;
      
      console.log(`Loan #${loan.id} repay eligibility:`, {
        isBorrower,
        hasLender,
        isNotRepaid,
        ...
        userAddress: address,
        canRepay: isBorrower && hasLender && isNotRepaid
      });
    });
  ...
  const canRepay = (loan) => {
    ...
    const isBorrower = isSameAddress(loan.borrower, address);
    const hasLender = loan.lender !== '0x0000000000000000000000000000000000000000';
    const eligibleForRepayment = isBorrower && hasLender && !loan.repaid;
    
    // More verbose logging when checking repayment eligibility
    console.log(`Checking repay eligibility for loan #${loan.id}:`, {
      isBorrower,
      borrowerAddress: loan.borrower, 
      userAddress: address,
      hasLender,
      lenderAddress: loan.lender,
      isRepaid: loan.repaid,
      canRepay: eligibleForRepayment
    });
    
    return eligibleForRepayment;
  };
